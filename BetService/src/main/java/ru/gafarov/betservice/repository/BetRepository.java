...
public interface BetRepository extends JpaRepository<Bet, Long> {
    ...
    @Query(value = "SELECT * FROM betbot.bets where status = 'ACTIVE' AND finish_date < ?1 AND id NOT IN (SELECT id FROM betbot.notify_expired_status where notify_status = 'NOTIFIED')"
            , nativeQuery = true)
